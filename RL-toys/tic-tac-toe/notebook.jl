...
# ╔═╡ e7c5ea40-442c-11ec-2bd6-1b129e97e7e7
begin
...
# ╔═╡ 595779c0-af00-4d4e-8654-71a285a9b2af
include("tic-tac-toe.jl")

# ╔═╡ df60943c-5f6d-4fc2-b358-676cfc7b7bcc
env = TicTacToeEnv()

# ╔═╡ 867d11ce-0783-4eff-9a39-2c362eef10a2
md"""
We use the monte carlo tree search for searching the strategy.
"""

# ╔═╡ e92b5dac-8217-4c13-9cd4-94b0e7ba011d
md"""
Wrap it with integer observation.
"""

# ╔═╡ ea659859-f24c-4e35-ad95-2213146c7e84
 E = DefaultStateStyleEnv{Observation{Int}()}(env)

# ╔═╡ 27b46d34-84c1-41e6-bbda-0d8c0a3dce40
md"""
First check the agent of both random policy.
"""

# ╔═╡ c47334f3-2f49-45b3-8119-db8f2e01c99f
begin
	x = RLEnvs.Cross()
	o = RLEnvs.Nought()
end

# ╔═╡ dfe0b2b1-a04f-4f92-ace5-557101d0f8a5
multi_random_policy = MultiAgentManager(
	NamedPolicy(ReinforcementLearningEnvironments.Nought() => RandomPolicy()),
	NamedPolicy(ReinforcementLearningEnvironments.Cross() => RandomPolicy()),
)

# ╔═╡ c934076a-1859-47da-af66-c9c3f9af2558
multi_random_hook = MultiAgentHook(
    (
		p => TotalRewardPerEpisode(is_display_on_exit=false)
		for p in players(env)
	)...
)

# ╔═╡ 0839944c-d5f9-41dd-8543-33f5bdc32c33
begin
	run(multi_random_policy, E, StopAfterEpisode(20), multi_random_hook)
	plot(multi_random_hook[x][]; label="x")
	plot!(multi_random_hook[o][]; label="o")
end

# ╔═╡ c702d326-9ca6-4821-839a-d8e1ef4ab306
sum(multi_random_hook[x][])

# ╔═╡ 67f701e3-57c0-4dd0-b183-b4f69fde52e2
md"""
About half the chance to win, but cross can win a little bit more.
"""

# ╔═╡ a1e943ee-1009-4993-95a1-f51156811f9d
    ...
    NamedPolicy(
		ReinforcementLearningEnvironments.Nought() => 
			MonteCarloTreeSearchPolicy(
				...
				episodes_per_search = 200,
			),
	),
    NamedPolicy(ReinforcementLearningEnvironments.Cross() => RandomPolicy()),
)

# ╔═╡ 8f0cf623-2551-4569-9a02-efc91eea9b10
...
# ╔═╡ aa489783-d967-4d85-99f2-5f664d070d18
run(multi_agent_policy, E, StopAfterEpisode(10), multi_agent_hook)

# ╔═╡ d7620fc0-5e36-428a-b7d8-64fd18d35420
begin
	plot(multi_agent_hook[x][]; label="x")
	plot!(multi_agent_hook[o][]; label="o")
end

# ╔═╡ b9555c1b-fdac-498c-aae5-657005485fb9
sum(multi_agent_hook[o][])

# ╔═╡ df54fe3d-2ddf-4bcd-ab7a-2ce1ce3864df
begin
	win_o_rand = []
	win_o_mcts = []
	for _ in 1:100
		
		for (_,v) in multi_random_hook.hooks
			empty!(v.rewards)
		end
		for (_,v) in multi_agent_hook.hooks
			empty!(v.rewards)
		end
		run(multi_random_policy, E, StopAfterEpisode(100), multi_random_hook)
		run(multi_agent_policy, E, StopAfterEpisode(100), multi_agent_hook)
		push!(win_o_rand, sum(multi_random_hook[o][]) / 100)
		push!(win_o_mcts, sum(multi_agent_hook[o][]) / 100)
	end
	
	plot(win_o_rand; label="random policy")
	plot!(win_o_mcts; label="MCTS")
end
...
git-tree-sha1 = "c88bce59c87151fd36e45f70f4b71573b6f2ca63"
...
git-tree-sha1 = "ef0d6fc7e1a32b3069970ef7262b87d7c9064bdb"
